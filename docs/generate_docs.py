...
def generate_technical_documentation():
    ...
    doc.add_paragraph(
        "• Architecture A: PPO (Proximal Policy Optimization) - Reinforcement Learning approach",
        style="List Bullet",
    )
    doc.add_paragraph(
        "• Architecture B: Isolation Forest - Traditional ML baseline", style="List Bullet"
    )
...
"""
    ...
    doc.add_paragraph(
        "• Each security event is treated independently (no sequential dependencies)",
        style="List Bullet",
    )
    doc.add_paragraph(
        "• Rewards are immediate based on action and ground truth label", style="List Bullet"
    )
    doc.add_paragraph(
        "• Actions are sampled once before training (offline/batch RL)", style="List Bullet"
    )
    ...
    doc.add_paragraph()
    doc.add_paragraph(
        "v128 Features (RL): Includes v7 features plus cyclic time encodings. Note: Only 11 of 128 dimensions are currently populated."
    )
...
"""
    ...
    doc.add_paragraph(
        "pip install numpy pandas torch scikit-learn pyyaml pydantic joblib", style="No Spacing"
    )
    ...
    doc.add_paragraph(
        "python -c \"from architecture_b_iforest.train import train_iforest; train_iforest('configs/iforest.yaml', 'configs/data.yaml')\"",
        style="No Spacing",
    )
    doc.add_paragraph()
    doc.add_paragraph("Train PPO:")
    doc.add_paragraph(
        "python -c \"from architecture_a_rl.train import train_ppo; train_ppo('configs/ppo.yaml', 'configs/data.yaml')\"",
        style="No Spacing",
    )
    ...
    doc.add_paragraph(
        "python -c \"from evaluation.head_to_head import run_head_to_head; run_head_to_head('configs/eval.yaml', 'configs/data.yaml', 'configs/iforest.yaml', 'configs/ppo.yaml')\"",
        style="No Spacing",
    )
    ...
    feature_errors = [
        [
            ...
            "Wrong feature length (v7=7, v128=128)",
            "Verify feature version and schema alignment",
        ],
        [
            "FEATURE_NAN_INF",
            "NaN or Inf detected in features",
            "Inspect upstream feature pipeline for invalid math or missing values",
        ],
        [
            "FEATURE_DTYPE",
            "Non-numeric feature values",
            "Ensure features are numeric floats (no strings/objects)",
        ],
    ]
