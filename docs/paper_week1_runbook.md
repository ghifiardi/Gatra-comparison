...
```bash
bash scripts/paper_matrix.sh --csv --dry-run
```

Run full CSV matrix (3 seeds):

```bash
bash scripts/paper_matrix.sh --csv --seeds 42,1337,2026
```
...
```bash
bash scripts/paper_matrix.sh --all --seeds 42,1337,2026 --bq-seeds 42
```
...
Using the index file generated by the runner:

```bash
python scripts/collect_paper_results.py \
  --index reports/paper_results/week1_run_index.csv \
