...
def test_run_cli_quick_smoke(tmp_path: Path) -> None:
    ...
    assert (run_dir / "contract" / "session_id_train.npy").exists()
    assert (run_dir / "contract" / "session_id_val.npy").exists()
    assert (run_dir / "contract" / "session_id_test.npy").exists()
    assert (run_dir / "contract" / "label_created_at_epoch_s_train.npy").exists()
    assert (run_dir / "contract" / "label_created_at_epoch_s_val.npy").exists()
    assert (run_dir / "contract" / "label_created_at_epoch_s_test.npy").exists()
    assert (run_dir / "contract" / "episode_id_train.npy").exists()
    assert (run_dir / "contract" / "episode_start_ts_train.npy").exists()
    assert (run_dir / "contract" / "episode_end_ts_train.npy").exists()
