...
class ToyDataset:
    ...
    def generate(self) -> tuple[list[RawEvent], list[Label]]:
        ...
        for i in range(self.n):
            ...
            port = int(rng.choice([80, 443, 22, 53, 3389, 8080], p=[0.3, 0.3, 0.1, 0.1, 0.1, 0.1]))
            protocol = str(rng.choice(["tcp", "udp"], p=[0.85, 0.15]))

            # inject threats with higher exfil + odd ports
            is_threat = rng.random() < 0.08
            if is_threat:
                bytes_sent *= float(rng.uniform(5, 20))
                port = int(rng.choice([4444, 1337, 5555, 3389], p=[0.4, 0.2, 0.2, 0.2]))
